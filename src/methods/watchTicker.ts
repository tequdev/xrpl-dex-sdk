...
import { BN, getBaseAmountKey, getOrderSideFromOffer, getQuoteAmountKey, parseMarketSymbol } from '../utils';
...
async function watchTicker(
  ...
  for (const bookOffer of bookOffers) {
    ...
    const side = getOrderSideFromOffer(bookOffer);

    const baseValue = BN(parseAmountValue(bookOffer[getBaseAmountKey(side)]));
    const quoteValue = BN(parseAmountValue(bookOffer[getQuoteAmountKey(side)]));
    ...
    if (side === 'buy') {
      ...
      askVolume = !askVolume || baseValue.isGreaterThan(askVolume) ? baseValue : askVolume;
    }
